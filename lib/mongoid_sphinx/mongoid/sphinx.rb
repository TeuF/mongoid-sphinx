  ...
  module Sphinx
    ...
    module ClassMethods
      ...
      def sphinx_stream
        ...
        self.all.entries.each do |document_hash|
          ...
          if sphinx_compatible_id > 0
            ...
            self.search_attributes.each do |key, value|
              value = case value
                when 'bool' then document_hash[key.to_s] ? 1 : 0
                when 'timestamp' then (document_hash[key.to_s].is_a?(Date) ? document_hash[key.to_s].to_time.to_i : document_hash[key.to_s].to_i)
                else
                  if document_hash[key.to_s].is_a?(Array)
                    document_hash[key.to_s].join(", ")
                  elsif document_hash[key.to_s].is_a?(Hash)
